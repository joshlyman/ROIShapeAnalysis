...
#             continue
#         if not fnmatch.fnmatch(dicomfile, '*dcm'):
#             continue
#
#         print dicomfile
...
# casefile = '/Pt3'
# dicomfile = '/Pt3 - DES - CC.dcm'

outcsv = '/Users/yanzhexu/Desktop/Research/ShapeBreatImageTest.csv'
...
def get_curvature(x, y):
    ...
    for i in range(len(dx)):
        ...
        if divi !=0:
            ...
            curvature = curvature**2
            curvlist.append(curvature)
        # curvature = np.abs(float(d2x_dt2[i] * dy[i] - dx[i] * d2y_dt2[i])) / float(divi)
        # curvlist.append(curvature)

    # div = (dx * dx + dy * dy) ** 1.5
    # # if div !=0:
    # curvature = np.abs(float(d2x_dt2 * dy - dx * d2y_dt2)) / float(div)
...
with open(outcsv, 'wb') as CSVFile:
    ...
    #diameter = np.max(v.max(axis=0) - v.min(axis=0))
    #print
    #lenpath = path_length(v)
    #print 'diameter',diameter
    #print lenpath
    #print len(c.collections)
    ...
    shapedescriptors = [dicomfile, Compactness, Entropy, bendingenergy, radialratio, perimeter, contourarea, normradius,
                        minradial, maxradial]
